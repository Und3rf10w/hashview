...
get '/pcaps/list' do
  ...
  @pcaps.each do |pcap|
    pcap_cracked_count = HVDB.fetch('SELECT COUNT(p.encodedhandshake) as count FROM handshakes p LEFT JOIN pcapfilehandshakes a ON p.id = a.handshake_id WHERE (a.pcap_id = ? AND p.cracked = 1)', pcap.id)[:count]
    pcap_cracked_count = pcap_cracked_count[:count]
    pcap_total_count = HVDB.fetch('SELECT COUNT(p.encodedhandshake) as count FROM handshakes p LEFT JOIN pcapfilehandshakes a ON p.id = a.handshake_id WHERE a.pcap_id = ?', pcap.id)[:count]
...
get '/pcaps/delete' do
  ...
  pcaphandshakes = HVDB[:pcapfilehandshakes]
